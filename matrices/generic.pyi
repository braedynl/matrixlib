...
if typing.TYPE_CHECKING:  # Avoids circular import
    from .numeric import IntegralMatrix
...
class Shape:
    ...
    def compatible(self, other: Shape, *, error: bool = False) -> bool: ...
    def copy(self) -> Shape: ...
...
class GenericMatrix(Sequence[T]):
    ...
    # These two operators will always return a GenericMatrix since you're
    # allowed to return other objects in their overloads (i.e., you're not
    # guaranteed a matrix of booleans). This is specified by the official data
    # model, but conflicts with object's definition. If a different subclass is
    # desirable (often an IntegralMatrix), use the refer() method.
    def __eq__(self: Self, other: GenericMatrix) -> GenericMatrix: ...  # type: ignore[override]
    def __ne__(self: Self, other: GenericMatrix) -> GenericMatrix: ...  # type: ignore[override]

    # Unlike the two above, the implementation for these guys guarantee boolean
    # results for consistency with logical not, so a promotion to an
    # IntegralMatrix is okay.
    def __and__(self: Self, other: GenericMatrix) -> IntegralMatrix[bool]: ...
    def __xor__(self: Self, other: GenericMatrix) -> IntegralMatrix[bool]: ...
    def __or__(self: Self, other: GenericMatrix) -> IntegralMatrix[bool]: ...

    def __rand__(self: Self, other: GenericMatrix) -> IntegralMatrix[bool]: ...
    def __rxor__(self: Self, other: GenericMatrix) -> IntegralMatrix[bool]: ...
    def __ror__(self: Self, other: GenericMatrix) -> IntegralMatrix[bool]: ...
    ...
    # MyPy will think that we've changed the global bool name if defined
    # earlier, so at the bottom it shall stay.
    def bool(self: Self) -> IntegralMatrix[bool]: ...
