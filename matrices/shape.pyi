...
class Shape(Collection[int]):
    ...
    def equal(self: Self, other: ShapeLike) -> bool: ...
    def copy(self: Self) -> Shape: ...
    def reverse(self: Self) -> Shape: ...
    def subshape(self: Self, *, by: int = Rule.ROW) -> Shape: ...
    def resolve_index(self: Self, key: SupportsIndex, *, by: int = Rule.ROW) -> int: ...
    def resolve_slice(self: Self, key: slice, *, by: int = Rule.ROW) -> range: ...
    def sequence(self: Self, index: int, *, by: int = Rule.ROW) -> tuple[int, int, int]: ...
    def range(self: Self, index: int, *, by: int = Rule.ROW) -> range: ...
    def slice(self: Self, index: int, *, by: int = Rule.ROW) -> slice: ...
