from abc import ABCMeta, abstractmethod
from collections.abc import Sequence
from typing import Any, Generic, Literal, TypeVar, Union, overload

__all__ = ["ShapeLike"]

M_co = TypeVar("M_co", covariant=True, bound=int)
N_co = TypeVar("N_co", covariant=True, bound=int)


class ShapeLike(Sequence[Union[M_co, N_co]], Generic[M_co, N_co], metaclass=ABCMeta):

    __slots__: tuple[()]
    __match_args__: tuple[Literal["nrows"], Literal["ncols"]]

    def __str__(self) -> str: ...
    def __lt__(self, other: ShapeLike) -> bool: ...
    def __le__(self, other: ShapeLike) -> bool: ...
    def __eq__(self, other: Any) -> bool: ...
    def __ne__(self, other: Any) -> bool: ...
    def __gt__(self, other: ShapeLike) -> bool: ...
    def __ge__(self, other: ShapeLike) -> bool: ...
    def __len__(self) -> Literal[2]: ...
    @overload  # type: ignore[override]
    @abstractmethod
    def __getitem__(self, key: Literal[0]) -> M_co: ...
    @overload
    @abstractmethod
    def __getitem__(self, key: Literal[1]) -> N_co: ...
    @overload
    @abstractmethod
    def __getitem__(self, key: int) -> Union[M_co, N_co]: ...
    @overload
    @abstractmethod
    def __getitem__(self, key: slice) -> Sequence[Union[M_co, N_co]]: ...

    @property
    def nrows(self) -> M_co: ...
    @property
    def ncols(self) -> N_co: ...

    def compare(self, other: ShapeLike) -> Literal[-1, 0, 1]: ...
