from abc import ABCMeta, abstractmethod
from collections.abc import Collection, Iterator
from typing import Any, Generic, Literal, Optional, TypeVar, overload

__all__ = ["ShapeLike"]

M_co = TypeVar("M_co", covariant=True, bound=int)
N_co = TypeVar("N_co", covariant=True, bound=int)

ShapeLikeT = TypeVar("ShapeLikeT", bound=ShapeLike)


class ShapeLike(Collection[M_co | N_co], Generic[M_co, N_co], metaclass=ABCMeta):

    __slots__: tuple[()]
    __match_args__: tuple[Literal["nrows"], Literal["ncols"]]

    def __str__(self) -> str: ...
    def __eq__(self, other: Any) -> bool: ...
    def __ne__(self, other: Any) -> bool: ...
    def __len__(self) -> Literal[2]: ...
    @overload
    @abstractmethod
    def __getitem__(self, key: Literal[0]) -> M_co: ...
    @overload
    @abstractmethod
    def __getitem__(self, key: Literal[1]) -> N_co: ...
    @overload
    @abstractmethod
    def __getitem__(self, key: int) -> M_co | N_co: ...
    def __iter__(self) -> Iterator[M_co | N_co]: ...
    def __reversed__(self) -> Iterator[M_co | N_co]: ...
    def __contains__(self, value: Any) -> bool: ...
    @abstractmethod
    def __deepcopy__(self: ShapeLikeT, memo: Optional[dict[int, Any]] = None) -> ShapeLikeT: ...
    def __copy__(self: ShapeLikeT) -> ShapeLikeT: ...

    @property
    def nrows(self) -> M_co: ...
    @property
    def ncols(self) -> N_co: ...

    def copy(self: ShapeLikeT) -> ShapeLikeT: ...
    @abstractmethod
    def reverse(self) -> ShapeLike[N_co, M_co]: ...
